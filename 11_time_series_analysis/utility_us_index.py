...
def naive_prediction(train_df, observations_to_predict):
    values = [train_df.iat[-1, 1] for i in range(observations_to_predict)]
    return create_data_frame(values, train_df.iat[-1, 0])


def average_prediction(train_df, observations_to_predict):
    values = [train_df["value"].mean() for i in range(observations_to_predict)]
    return create_data_frame(values, train_df.iat[-1, 0])


def sarima_prediction(train_df, observations_to_predict):
    order = (1, 1, 1)
...
def prophet_prediction(train_df, observations_to_predict):
    prophet_model = prophet.Prophet()
    prophet_model.fit(train_df.rename(columns={"date": "ds", "value": "y"}))
    future = prophet_model.make_future_dataframe(periods=observations_to_predict)
    forecast = prophet_model.predict(future)
    values = forecast["yhat"].iloc[-observations_to_predict:]
    return create_data_frame(values, train_df.iat[-1, 0])
...
utility_index_df = utility_index_df[(utility_index_df["date"] >= pd.Timestamp("1979-01-01")) & (utility_index_df["date"] < pd.Timestamp("2019-01-01"))]
...
# print("SARIMA", cross_validate(utility_index_df, cross_validation_splits, sarima_prediction))
