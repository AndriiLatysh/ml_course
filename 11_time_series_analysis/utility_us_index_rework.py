...
def sarima_prediction(train_df, observation_to_predict, **kwargs):
    sarima_model = statmodels_sarimax.SARIMAX(train_df, order=kwargs["order"], seasonal_order=kwargs["seasonal_order"], freq="MS")
...
#     (utility_index_df.index >= pd.Timestamp("1979-01-01")) & (utility_index_df.index < pd.Timestamp("2019-01-01"))]
...
plot_cv_prediction(naive_predictions)
...
plot_cv_prediction(average_predictions)
...
utility_index_additive_decomposition = statsmodels_seasonal.seasonal_decompose(utility_index_df, model="additive", freq=12)
utility_index_additive_decomposition.plot()
utility_index_multiplicative_decomposition = statsmodels_seasonal.seasonal_decompose(utility_index_df, model="multiplicative", freq=12)
